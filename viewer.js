...
function setViewBox(vb,immediate=false){ if(!svgEl)return; viewCur=vb; pendingView=vb; const commit=()=>{viewRaf=0;const v=pendingView;pendingView=null;if(v&&svgEl)svgEl.setAttribute('viewBox',`${v.x} ${v.y} ${v.w} ${v.h}`);}; if(immediate){if(viewRaf)cancelAnimationFrame(viewRaf);commit();}else if(!viewRaf)viewRaf=requestAnimationFrame(commit);}
function clientToSvgXY(cx,cy,vb=viewCur){ if(!svgEl||!vb)return{x:0,y:0};const r=svgEl.getBoundingClientRect();if(!r.width||!r.height)return{x:0,y:0};return{x:vb.x+(cx-r.left)/r.width*vb.w,y:vb.y+(cy-r.top)/r.height*vb.h}; }
...
// --- Ortskunde: nur Tanklager anzeigen ---
function renderOrtskunde(){
  ...
  // Filter: Nur Namen, die "Tanklager" enthalten ODER mit "TL " beginnen
  const onlyTL = $('#chkTL') ? $('#chkTL').checked : true;
  if(onlyTL){
    const re = /(^TL\b|\bTanklager\b)/i;
...
$('#svgObj').addEventListener('load', () => {
  ...
  viewBase = getInitialViewBox(svgEl); setViewBox({...viewBase},true); wirePanZoom();
  if (jsonData) apply();
});
...
$('#chkTL')?.addEventListener('change', ()=> renderOrtskunde());
